...
The system SHALL provide a view helper that takes an 18-char hashed ID and renders it as a three-color visual identity. The hash SHALL be split into three 6-character segments. Each segment SHALL be treated as an HTML hex color. The rendered output SHALL contain the hash text with each segment colored in its own color (as inline CSS `color` style), separated by hyphens, followed by three colored square glyphs (U+25A0, `■`).

#### Scenario: Hash renders as colored segments with squares
- **WHEN** the identity renderer is called with hash `a7f3b2c1e9d04f8baa`
- **THEN** the output contains three `<span>` elements with inline styles `color:#a7f3b2`, `color:#c1e9d0`, and `color:#4f8baa`, displaying text `a7f3b2`, `c1e9d0`, and `4f8baa` respectively, separated by hyphens, followed by three colored square glyphs
...
The account page SHALL display the user's hashed ID rendered as the three-color visual identity (colored text segments plus colored squares). The page SHALL provide export data, delete account, and logout functionality. The page SHALL NOT display any GitHub profile information.

#### Scenario: Account page renders for logged-in user
- **WHEN** a logged-in user visits `/account`
- **THEN** the page shows their three-color hash identity, an export data link, a delete account button, and a logout link
